module Chess
  class Game
    ...
    def move_flow
      ...
      if king_check > 0
        if check_checkmate == true
          ...
          puts "Your king is in check. You must get your king out of check with your next move."
        end
      elsif draw_check == true
        puts "#{current_player.name} has no available moves. The game has ended in a draw."
        exit
      end
      @move = get_move
    ...
    def check_check(piece, origin, dest)
      ...
      puts "That move would put your king in check" if threat_count > 0 && current_board == @board
      threat_count
    end
        ...
        row.each do |cell|
          if (cell.value != nil) && (cell.value.color == player.color)
            cell.value.possible_moves.each do |move|
              ...
              if piece.name == "pawn"
                if pawn_move_check(piece.position, move, piece, dest_value, @move_list.last, current_board) != false
    ...
    def pawn_move_check(origin, dest, piece, dest_value, last_move, current_board=nil)
      current_board ||= @board
      if (last_move != nil && last_move[:name] == 'pawn') && (dest[0] != origin[0]) &&
        ...
        if ((piece.color == 'white') && (origin[1] == 3) && (last_move[:origin][1] == 1) && (last_move[:dest][1] == (last_move[:origin][1] + 2))) ||
          ...
          current_board.set_cell(last_move[:dest][0], last_move[:dest][1], nil) if current_board == @board
          return true
        end
